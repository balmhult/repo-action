...
# JST timezone offset
JST_hour_offset=9
current_hour_JST=$(( (current_hour + JST_hour_offset) % 24 ))

if [ "$current_day" -ge 1 ] && [ "$current_day" -le 5 ]; then
  if [ "$current_hour_JST" -ge 9 ] && [ "$current_hour_JST" -lt 18 ]; then
    echo "[+] Action start"
    ...
    echo "[+] Outside of working hours. Exiting."
  fi
else
  echo "[+] Outside of working days. Exiting."
fi